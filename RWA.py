...
if not df_h.empty: total_r = safe_f(df_h['Profit_Realized'].sum())
for cat in STRATEGY.values():
    for name, info in cat.items():
        ...
        h, e = safe_f(u_row['Holdings'].iloc[0]) if not u_row.empty else 0.0, safe_f(u_row['Entry_Price'].iloc[0]) if not u_row.empty else 0.0
