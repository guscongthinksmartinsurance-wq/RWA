...
# FIXED: Mapping để lấy giá chuẩn nhưng vẫn giữ tên hiển thị theo ý anh
SYMBOL_MAP = {
...
with st.sidebar:
    ...
    st.divider()
    # Danh sách gợi ý bao gồm cả mã anh đã nhập trong sheet
...
# --- 5. LẤY DỮ LIỆU & RENDER ---
# FIXED: Thu thập tất cả coin từ Sheet và Chiến lược để hiển thị
...
for coin in all_coins:
    ...
    try:
        ...
        if coin in RWA_STRATEGY:
            card.update({"tw": RWA_STRATEGY[coin]['target_w'], "rw": (val/2000*100), "fill": min((val/2000*100)/RWA_STRATEGY[coin]['target_w'], 1.0)*100})
