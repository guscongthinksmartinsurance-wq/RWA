...
def analyze_advanced_logic(df, cp, days_sel, has_holdings, pnl_pct):
    ...
    return sup, res, stt, col, " | ".join(checks), float(df['High'].max()), cp*1.5, cp*2.0, eff
...
# --- 4. SIDEBAR PHỤC HỒI 100% ---
with st.sidebar:
    st.header("🏢 MANAGEMENT")
    base_budget = st.number_input("TOTAL BUDGET ($)", value=2000.0)
    st.divider()
    all_known = sorted(list(set(list(RWA_STRATEGY.keys()) + list(MAP.keys()) + df_holdings['Coin'].tolist())))
...
# PHÂN TÍCH
all_assets = list(set(list(RWA_STRATEGY.keys()) + [c for c in df_holdings['Coin'].tolist() if c]))
...
# DASHBOARD (Dấu ngoặc đã được đóng chuẩn 100%)
...
# TABS PHỤC HỒI 7 CỘT NO-ICON & 4 CHỈ SỐ ✅/❌
t1, t2 = st.tabs(["🛡️ STRATEGIC RWA", "🔍 HUNTER SCANNER"])
def render_v17(data, is_rwa):
    for d in data:
...
with t1: render_v17(tab1_data, True)
with t2: render_v17(tab2_data, False)
