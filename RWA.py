...
if not df_h.empty:
    for _, row in df_h.iterrows():
        # Tìm ID từ STRATEGY dựa trên Ticker (Coin)
        c_id = next((v['id'] for cat in STRATEGY.values() for k, v in cat.items() if k == row['Coin']), None)
        if c_id:
            current_p = prices.get(c_id, {}).get('usd', 0)
            total_value += row['Holdings'] * current_p
            total_pnl += (current_p - row['Entry_Price']) * row['Holdings']

# 2. FINANCIAL DASHBOARD (TRẠM CHỈ HUY)
st.title("🛡️ SOVEREIGN COMMAND CENTER")
d1, d2, d3, d4 = st.columns(4)

with d1:
    st.markdown(f'<div class="header-box"><div class="metric-label">Tổng Tài Sản</div><div class="metric-value">${total_value:,.2f}</div></div>', unsafe_allow_html=True)
with d2:
...
def render_tab(category, tab_obj):
    with tab_obj:
        ...
        for i, (symbol, info) in enumerate(STRATEGY[category].items()):
            ...
            chg = prices.get(info['id'], {}).get('usd_24h_change', 0)
            ...
            with cols[i % 4]:
                ...
                <div class="coin-card">
                    ...
                    <h2 style="margin:10px 0;">${cp:,.4f} <small style="font-size:12px; color:{'#3fb950' if chg > 0 else '#f85149'};">{chg:+.2f}%</small></h2>
