...
# Mapping cưỡng bức để ARB và SUI không bao giờ sai giá
SYMBOL_MAP = {
    'ARB': 'ARB1-USD', 'SUI': 'SUI-USD', 'SOL': 'SOL-USD', 
    'SEI': 'SEI-USD', 'FET': 'FET-USD', 'PEPE': 'PEPE1-USD'
}

# --- 2. HÀM TRỢ NĂNG (NEWS & LOGIC) ---
...
def analyze_advanced_logic(df, cp, days_sel, has_holdings, pnl_pct):
    ...
    elif rsi < 35 or cp <= lower_b: stt, col = "STRONG BUY", "#3fb950"
    else: stt, col = "OBSERVE", "#8b949e"
    
    rs_text = f"RSI: {rsi:.1f} | VOL: x{vol:.1f}"
    return sup, res, stt, col, rs_text, float(df['High'].max()), cp*1.5, cp*2.0, eff_score
...
with st.sidebar:
    ...
    st.divider()
    all_coins_in_list = sorted(list(set(list(RWA_STRATEGY.keys()) + list(SYMBOL_MAP.keys()) + df_holdings['Coin'].tolist())))
    coin_sel = st.selectbox("Select Asset", ["+ New Asset..."] + all_coins_in_list)
    final_coin = st.text_input("Symbol (e.g. ARB, SUI)").upper() if coin_sel == "+ New Asset..." else coin_sel
    
    with st.form("trade_v13"):
        ...
        if st.form_submit_button("EXECUTE"):
            if final_coin:
                ...
                if not row.empty:
                    ...
                    t_q, a_e = old_q + q, ((old_q * old_e) + (q * p)) / (old_q + q) if (old_q + q) > 0 else 0
...
# LẤY DỮ LIỆU & RENDER (PHỤC HỒI HIỂN THỊ)
all_coins = list(set(list(RWA_STRATEGY.keys()) + df_holdings[df_holdings['Holdings'] > 0]['Coin'].tolist()))
if not all_coins: all_coins = list(RWA_STRATEGY.keys())
...
with t1: render_v13(tab1_data, True)
with t2: render_v13(tab2_data, False)
