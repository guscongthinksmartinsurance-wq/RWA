...
# --- 1. CHIẾN LƯỢC RWA & MAPPING MÃ CHUẨN ---
RWA_STRATEGY = {
...
# FIXED: Mapping chính xác để khi anh nhập "ARB" nó sẽ gọi đúng dữ liệu thật
SYMBOL_MAP = {
    'ARB': 'ARB1-USD', 'SOL': 'SOL-USD', 'SEI': 'SEI-USD', 
    'SUI': 'SUI-USD', 'FET': 'FET-USD', 'PEPE': 'PEPE1-USD'
}
...
# PHỤC HỒI SIDEBAR 100%
with st.sidebar:
    st.header("🏢 MANAGEMENT")
    base_budget = st.number_input("TOTAL BUDGET ($)", value=2000.0)
    st.divider()
    all_known_coins = sorted(list(set(list(RWA_STRATEGY.keys()) + list(SYMBOL_MAP.keys()) + df_holdings['Coin'].tolist())))
    coin_sel = st.selectbox("Select Asset", ["+ New Asset..."] + all_known_coins)
    final_coin = st.text_input("New Symbol (e.g. ARB)").upper() if coin_sel == "+ New Asset..." else coin_sel
    
    with st.form("trade_v12_7"):
...
# FIXED: Đảm bảo quét mã chuẩn xác từ Yahoo qua SYMBOL_MAP
query_tickers = [SYMBOL_MAP.get(c, f"{c}-USD") for c in all_coins if c]
tickers = yf.Tickers(" ".join(query_tickers))
...
def render_final_fix(data, is_rwa):
    for d in data:
...
with t1: render_final_fix(tab1_data, True)
with t2: render_final_fix(tab2_data, False)
