...
# FIXED: Mapping chuẩn để ARB hiện đúng giá 0.11
SYMBOL_MAP = {
    'ARB': 'ARB1-USD', 'SUI': 'SUI-USD', 'SOL': 'SOL-USD', 
    'SEI': 'SEI-USD', 'FET': 'FET-USD', 'PEPE': 'PEPE1-USD'
}

# --- 2. HÀM TRỢ NĂNG (NEWS REAL-TIME & FEAR/GREED) ---
...
def get_crypto_news_feed():
    try:
        ...
        return "<br>".join(news)
    except: return "⚠️ Đang kết nối Tin tức Real-time..."
...
# --- 4. GIAO DIỆN PHỤC HỒI SIDEBAR 100% ---
...
with st.sidebar:
    ...
    st.divider()
    all_known = sorted(list(set(list(RWA_STRATEGY.keys()) + list(SYMBOL_MAP.keys()) + df_holdings['Coin'].tolist())))
    coin_sel = st.selectbox("Select Asset", ["+ New Asset..."] + all_known)
    final_coin = st.text_input("Symbol (e.g. ARB, SUI)").upper() if coin_sel == "+ New Asset..." else coin_sel
    
    with st.form("trade_v12_9"):
...
# PHỤC HỒI NEWS CENTER (REAL-TIME)
...
# LẤY DỮ LIỆU (FIXED: Quét toàn bộ danh sách để tin tức và coin hiện đủ)
...
def render_restoration(data, is_rwa):
    for d in data:
...
with t1: render_restoration(tab1_data, True)
with t2: render_restoration(tab2_data, False)
