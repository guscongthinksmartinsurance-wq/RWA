...
# --- 3. BỘ NÃO PHÂN TÍCH 4 CHỈ SỐ (CHECKLIST) ---
...
# SIDEBAR GỘP THÔNG MINH
with st.sidebar:
    st.header("🏢 TỔNG TRẠM")
    budget = st.number_input("VỐN CHIẾN DỊCH ($)", value=2000.0)
    st.divider()
    st.write("🎯 **LỆNH MỚI**")
    ...
    with st.form("dca_final"):
        ...
        p_add = st.number_input("Giá ($)", min_value=0.0)
...
for coin in all_coins:
    ...
    try:
        ...
        card = {"coin": coin, "cp": cp, "stt": stt, "col": col, "rs": rs, "invested": invested, "e": e, "pnl": ((cp/e)-1)*100 if e>0 else 0, "sup": sup, "res": res, "ath": ath, "tp1": tp1, "tp2": tp2}
...
with col_d1:
    dash_html = f"""<div style="display:flex;gap:15px;font-family:sans-serif;margin-bottom:15px;"><div style="flex:1;background:#161b22;padding:25px;border-radius:20px;border:1px solid #30363d;text-align:center;"><div style="color:#8b949e;font-size:12px;text-transform:uppercase;">Cash Còn Lại</div><div style="color:#58a6ff;font-size:42px;font-weight:900;">${(budget-total_invest):,.0f}</div></div><div style="flex:1;background:#161b22;padding:25px;border-radius:20px;border:1px solid #30363d;text-align:center;"><div style="color:#8b949e;font-size:12px;text-transform:uppercase;">Lời / Lỗ</div><div style="color:{'#3fb950' if pnl_total>=0 else '#f85149'};font-size:42px;font-weight:900;">${pnl_total:,.0f}</div></div></div><div style="background:#161b22;padding:25px;border-radius:20px;border:1px solid #30363d;text-align:center;"><div style="color:#8b949e;font-size:12px;text-transform:uppercase;">Tổng Giá Trị Tài Sản</div><div style="color:white;font-size:48px;font-weight:900;">${total_val:,.0f}</div></div>"""
...
with t1: render_ui(tab1_data, True)
with t2: render_ui(tab2_data, False)
