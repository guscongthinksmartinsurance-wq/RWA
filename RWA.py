...
# --- 1. BASELINE: 11 CHIẾN MÃ 2026 (CoinGecko IDs) ---
STRATEGY = {
    'RWA': {
        ...
        'SYRUP': {'id': 'maple', 'id_alt': 'syrup-usdc', 'tw': 10, 'ath': 2.10},
...
# --- 2. HÀM TRỢ NĂNG (FEAR & GREED + NEWS) ---
...
# LẤY GIÁ & CHỈ SỐ QUA COINGECKO
ids = ",".join([v['id'] for cat in STRATEGY.values() for v in cat.values()])
price_data = requests.get(f"https://api.coingecko.com/api/v3/simple/price?ids={ids}&vs_currencies=usd&include_24hr_change=true&include_market_cap=false").json()
...
for cat, coins in STRATEGY.items():
    for name, info in coins.items():
        ...
        # 4 CHỈ SỐ KỸ THUẬT (Mô phỏng dựa trên biến động 24h & support)
        score = 0
        checks = []
        if ch24h < -3: score += 1; checks.append("✅ RSI LOW")
        else: checks.append("❌ RSI MID")
        if ch24h < -1: score += 1; checks.append("✅ BB BOTTOM")
        ...
        elif score == 2: stt, col = "SPEC BUY" if h == 0 else "DCA BUY", "#1f6feb" if h > 0 else "#58a6ff"
        ...
        card = {"name": name, "cp": cp, "stt": stt, "col": col, "rs": " | ".join(checks), "inv": inv, "e": e, "pnl": pnl_hold, "sup": cp*0.94, "res": cp*1.15, "ath": info['ath'], "eff": (pnl_hold/12 if h > 0 else 0)}
...
def render_v18_8(data, is_rwa):
    for d in data:
        # LOGIC PRECISION GIÁ: Pepe hiện đủ số 0
        p_fmt = f"{d['cp']:.6f}" if d['cp'] < 0.001 else f"{d['cp']:.3f}"
...
with t1: render_v18_8(tabs_data['RWA'], True)
with t2: render_v18_8(tabs_data['HUNTER'], False)
