...
# FIX TRIỆT ĐỂ BÁO GIÁ: Ép mã chuẩn xác cho Arbitrum và các đồng Hunter
HUNTER_SYMBOLS = {
...
with st.sidebar:
    ...
    st.divider()
    c_list = sorted(list(set(list(RWA_STRATEGY.keys()) + list(HUNTER_SYMBOLS.keys()) + df_holdings['Coin'].tolist())))
...
for coin in all_coins:
    ...
    try:
        ...
        group = RWA_STRATEGY[coin]['group'] if coin in RWA_STRATEGY else HUNTER_SYMBOLS.get(coin, 'Other')
...
# --- TAB RENDER (PHỤC HỒI THANH TỶ TRỌNG & HIỆU QUẢ) ---
t1, t2 = st.tabs(["🛡️ STRATEGIC RWA", "🔍 HUNTER SCANNER"])
def render_v12_6(data, is_rwa):
    for d in data:
        ...
        html = f"""<div style="background:#161b22;padding:25px;border-radius:20px;border:1px solid #30363d;font-family:sans-serif;color:white;margin-bottom:20px;"><div style="display:flex;justify-content:space-between;align-items:center;"><div style="width:50%;"><div style="font-size:36px;font-weight:900;color:#58a6ff;">{d['coin']}</div>{progress}</div><div style="text-align:right;"><div style="font-size:46px;font-weight:900;">${d['cp']:.3f}</div><div style="color:{'#3fb950' if d['pnl']>=0 else '#f85149'};font-size:22px;font-weight:800;">{d['pnl']:+.1f}%</div></div></div><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:8px;text-align:center;background:rgba(0,0,0,0.3);padding:20px;border-radius:15px;margin-top:20px;"><div><div style="color:#8b949e;font-size:10px;">INVESTED</div><div style="font-size:15px;font-weight:700;color:#58a6ff;">${d['invested']:,.0f}</div></div><div><div style="color:#8b949e;font-size:10px;">AVG</div><div style="font-size:15px;font-weight:700;">${d['e']:.3f}</div></div><div><div style="color:#8b949e;font-size:10px;">SUPPORT</div><div style="font-size:15px;font-weight:700;color:#3fb950;">${d['sup']:.3f}</div></div><div><div style="color:#8b949e;font-size:10px;">RESISTANCE</div><div style="font-size:15px;font-weight:700;color:#f85149;">${d['res']:.3f}</div></div><div><div style="color:#8b949e;font-size:10px;">ATH</div><div style="font-size:15px;font-weight:700;color:#d29922;">${d['ath']:.1f}</div></div><div><div style="color:#8b949e;font-size:10px;">TP1</div><div style="font-size:15px;font-weight:700;color:#3fb950;">${d['tp1']:.2f}</div></div><div><div style="color:#8b949e;font-size:10px;">TP2</div><div style="font-size:15px;font-weight:700;color:#d29922;">${d['tp2']:.2f}</div></div></div><div style="margin-top:20px;padding:15px;border-radius:12px;border-left:8px solid {d['col']};background:{d['col']}15;color:{d['col']};font-weight:800;font-size:18px;">{d['stt']}<br><span style="font-size:13px;font-weight:400;color:#f0f6fc;">{d['rs']}</span></div></div>"""
        components.html(html, height=410)

with t1: render_v12_6(tab1_data, True)
with t2: render_v12_6(tab2_data, False)
